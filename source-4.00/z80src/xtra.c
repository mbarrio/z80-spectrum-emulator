...
You ought to get no warnings, not even three 'possibly incorrect assignment' ones.
*/
...
extern char far microdrive_buffer[17440];   /* zie mdrvbuf.8 */
extern unsigned int count200hz; /* registered! */
int         pestmode=0;
...
const char  tzxtzx[]="TZX";
const char  vgamessage[]="If this is visible, use switch -xv or -g";
const char	helpfile[]="LAYOUT.SCR";
const int	ynalist[]={'A'};
const int	ynlist[]={'J','Y','N',27,0};
const int   ynentlist[]={'J','Y','N',13,27,0};
const int	mainmenu[]={2,'M','B','R','L','S','T','C','H','X','O','Q',
                        'b','c','d','f','g','h','i',27,0};
const int   changelist[]={'L','H','R','F','C','J','2','V','S','I','O','K',27,0};
const int   tapelist[]={'T','P','S','I','O','F','R','W','M','B','D','X','B','g',27,0};
...
extern long     TapeInFilePos, TapeInBlockPos;
extern char     gszTzxFile[129];
...
void            prgetal(unsigned long,int);
...
word            unpackfile(word,byte*,word,word);
void			save_prog(void);
void            tape(char);
void			mdrv(void);
void			change(void);
...
extern void far         resetdrive(int);
extern void far         qualify(char*, char*);

void            readtzxfile(long);              // In tzx.c
void            CloseTzxFile(void);
int             OpenTzxFile(void);
int             ReadBuffer(byte *, int);
void            PrintTzxBuf(byte *, int);
...
{
	  int i;
      for (i=0;vgamessage[i];i++) printchar(i,30,vgamessage[i]);
}
          ...
          case 'Z':
			   ...
			   switch (ch2) {
				   ...
				   case 'S':
						...
						break;
				   case 'V':
                        return(5);     /* voc file playback only reg users */
                        setvocvars:
                        vocplay=0xff;
                        srate=100;
						vocdatapoint=0;
...
char	ch;
{
	  if (ch==13) {
          ...
          if (xcurs>=xwmax2) xcurs=xwmax2-1;
	  }
}
...
word unpackfile(handle,outp,size,maxout)
// unpacks 'size' input bytes, but stores at most 'maxout' bytes
word handle;
byte *outp;
word size;
word maxout;
{
    word incount=0,outcount=0;
    word bufcount=0,bufptr=0;
    #define inpbf ((unsigned char*)microdrive_buffer)
    word i,length;
    byte j;
    char edflag=0;
    flush_mdrv_buffer();        /* ensure we can use this buffer */
    do {
       if (bufptr==bufcount) {
            i=size-incount;
            if (i>8192) i=8192;
            read_file(handle,inpbf,i);
            bufcount=i;
            bufptr=0;
       }
       if (edflag==2) {
            ...
            outcount+=length-2;
            if (outcount>maxout) return(maxout);
            for (;length!=0;length--) *(outp++)=j;
            edflag=0;
       } else {
            if (inpbf[bufptr]==0xED) edflag++; else edflag=0;
            incount++;
            outcount++;
            if (outcount>maxout) return(maxout);
            *(outp++)=inpbf[bufptr++];
...
char *fil1,*fil2;
{
	  ...
	  for (t=1;*(fil1+t);t++) {
          if (upc(*(fil1+t))<upc(*(fil2+t))) return(1);
          if (upc(*(fil1+t))>upc(*(fil2+t))) return(0);
	  }
	  return(1);
}
				  ...
				  default:
                       if (((n>='a')&&(n<='z'))||
						   ...
						   while ((*(curnam+16))&&
                                  ...
                                  (j<100)) {
                               for (j=1;j<fast;j++)
                                   if(upc(*(curnam+j))-upc(*(curnam+j+16))) j=100;
                               if (j<100) {
                                   x^=1;
                                   if (!x) y++;
                                   if (y>=ywmax2-6) {
                           ...
                           if ((upc(*(curnam+fast))!=n)||(j>=100)) {
                              if (fast==1) fast=0; else {
                                fast=0;
                                goto first_letter;
                              }
                           }
...
void change_input()
{
      change_menu("Redir If1 RS232 Input",&inmode,&innum,inputfile);
}

void change_output()
{
      change_menu("Redir If1 RS232/MGT Centr Output",&outmode,&outnum,outputfile);
}
...
void change(void)
{
	  ...
	  do {
          ...
          if (Double<0) prat(25,10,"50"); else {
            xcurs=25;
            ycurs=10;
            prgetal((100+(Double>>1))/Double,1);
          }
          prstr(" Hz");
          if (((Double!=2)&&(Double>0))||(syncmod==2)) prat(25,11,"Normal"); else
...
{
	  int i,j;
      if (count200hz>=(unsigned int)-11) {
        pestmode++;
        count200hz=0;
        clearwindow();
        prcenter(1,"Sinclair ZX Spectrum emulator");
        prcenter(2,"'Z80'");
        prcenter(3,"Shareware version 4.00");
        prcenter(4,"(January 1999)");
        if (pestmode==1) {
            prat(1,6,"This program will run for about");
            prat(1,7,"five minutes, with only a few");
            prat(1,8,"features - like tape-loading and");
            prat(1,9,"Disciple emulation - disabled.");
            i=10;
            do {
                xcurs=xwmax/2-1;
                ycurs=ywmax-2;
                ...
                prw(' ');
                prwind();
                for (j=0;j<18;j++) pauze();
                i--;
            } while (i>=0);
            do {} while (getkey());
            clearkeyboard();
        } else if (pestmode==2) {
            prat(1,6,"Please read the REGISTER.DOC");
            prat(1,7,"file for details on how to");
            prat(1,8,"register for this program.");
            prat(1,9,"  From now on, the emulator");
            prat(1,10,"cannot be slowed down.");
            prwind();
            for (j=0;j<18*2;j++) pauze();
            prat(xwmax-7,ywmax-2,"Key...");
            prwind();
            do {} while (getkey());
            do {} while (!getkey());
            speed_convert("500");
...
void help()
{
      ...
      prat (1,4,"ALT-F1  Keyboard lay-out");
	  ...
	  prcenter (5,"Emulator");
      prcenter (7,"Version 4.00");
      prcenter (8,"January 1999");
...
void browsetapefile(char tzx)
//
// tzx==0 : browse tape file
// tzx==1 : browse tzx file
// tzx==2 : make selection in tzx selection block
//
{
	  unsigned int plow,phigh,i,j,length;
      int teller,position,firstlin=0,ypos=0,maxn,n;
	  unsigned char *bufp1;
      int handle=tapeinh;

      if (tzx) handle=OpenTzxFile();

      // entry after delete; happens only if tzx==0
      ...
      prat (1,1,tzx==2 ? "TZX: Which part d'ye wanna play?" :
                         "Position the tape pointer:");
      if (!tzx) prat (3,2,"(press DEL to delete block)");
      lseek(handle,&plow,&phigh,1);

      // Move to beginning of file, unless browsing selection block
      i=j=0;
      if (tzx != 2)
          lseek(handle,&i,&j,0);

      // Now read relevant info from tap/tzx blocks
      // n is maximum number of elements read
      if (tzx == 2) {
        // first read max number (1st 2 bytes is total len)
        // invalidate file pointer
        read_file(handle,buffer,3);
        maxn = ((unsigned char*)buffer)[2];
        TapeInFilePos=-1;
      } else {
        maxn = 500;
      }
      ...
      do {
          bufp1=buffer+32*min(maxn,teller);
		  i=j=0;
          lseek(handle,&i,&j,1);
          bufp1[28]=(i & 0xff);
          bufp1[29]=(i >> 8);
          ...
          switch (tzx) {
          case 1:
              position = TapeInBlockPos;
              if (ReadBuffer(bufp1, handle))
                length=0xffff;
              else
                length=0;
              break;
          ...
          case 2:
              ...
              read_file(handle,(char*)&bufp1[30],2);
              // read length text
              i=0;
              read_file(handle,(char*)&i,1);
              // read text (max 30)
              read_file(handle,(char*)bufp1,i);
              bufp1[i<30?i:29]=0;
              break;
          }
          teller++;
      ...
      // main loop.  First print visible window of tape/tzx file
	  ...
	  do {
		  ...
		  for (i=0;i<ywmax2-6;i++) {
			  ...
			  if (firstlin+i>=teller) {
                  ...
                  // Here's the code that converts buffered data into humane format

                  if (i+firstlin>=maxn) {
                    prat(3,i+4,"No more memory! (block ");
                    prgetal(i+firstlin,1);
                    prw(')');
                  } else {
                    switch (tzx) {
                    case 1:
                      PrintTzxBuf(bufp1, i+4);
                      break;
                    case 0:
                      if (i+firstlin==teller-1)
                        prat(3,i+4,"<End of file>");
                      else {
                       switch(bufp1[0]) {
                        case 0:
						   switch(bufp1[1]) {
		  ...
		  switch (n) {
			  ...
			  case 339:
				   ...
				   clrlin(2);
				   prwind();
                   // We might be reading & saving to same file.
                   // If so, close output file first, re-open after
                   // deleting.
                   if (tapeouth)
                    close_file(tapeouth);
				   bufp1=buffer+32*(ypos+firstlin);
                   i=bufp1[28]+(bufp1[29]<<8);
                   j=bufp1[30]+(bufp1[31]<<8);
                   // first re-open file in read-write mode
                   close_file(handle);
                   chdir(tapdir);
                   handle = open_file(tapein,2);
                   if (handle==-1) {
                    prat(1,2,"File is read only.  Key..");
                    prwind();
                    do {} while (!getkey());
                    goto tap_nodel;
                   }
                   ...
                   // close 'read-write' file, re-open file in r/o mode
                   close_file(handle);
                   tap_nodel:
                   handle = open_file(tapein,_tapeinmode);
                   lseek(handle,&i,&j,0);
                   // Now reopen output file if necessary.
                   if (tapeouth) {
                    tapeouth = open_file(tapeout,2);
                    // goto end of file
                    lseek(tapeouth,&i,&j,2);
                   }
                   chdir(defdir);
				   goto browse_again;
			  default:
      ...
      // reset tap/tzx variables to start reading from selected block
      bufp1=buffer+32*(ypos+firstlin);
      switch (tzx) {
      case 1:
          TapeInBlockPos=ypos+firstlin;
          TapeInFilePos=-1;
          break;
      case 0:
          tapeeof=0;
...
{
	  char tempnaam[129];
      char menumode=0,level0;
	  char ch;
	  int i,voc_winded=0,wspd=0,loopcnt;
	  unsigned int p,q,voc_pos,voc_len;
	  unsigned long t;

      if (vochandle) menumode=1;
      if (gszTzxFile[0]) menumode=2;
      if (mode) {
        ...
        if (vochandle) {
            ...
            menumode=vochandle=loadpause=vocplay=voc_winded=0;
        }
        if (gszTzxFile[0]) {
            gszTzxFile[0]=0;
            menumode=vocplay=0;
      ...
      do {
		  tapeagain:
          if (menumode<level0) level0=menumode;
		  clearwindow();
		  prat (1,1,"Tape Interface settings");
          if (menumode==1) {                     /* VOC file submenu */
			  ...
			  switch (ch) {
				  ...
				  case 27:
                       menumode=0;
                       if (!level0) ch=0;    // escape out if not visited main tape menu yet
                       case27:
					   if (voc_winded) {
						   i=windvoc(voc_pos);
          ...
          } else if (menumode==2) {         // tzx file submenu
              prat (1,3,"T: TZX file to play:");
              if (gszTzxFile[0]) prat_file(3,4,gszTzxFile); else prat(3,4,"none");
			  prat (1,6,"S: Recorder status:");
              if (gszTzxFile[0]) {
				  if (vocplay)
					   prat (3,7,"Playing");
			  ...
			  switch (ch) {
                  case 'T':
					   xcurs=3;ycurs=4;
					   prwind();
                       for (i=0;i<129;i++) tempnaam[i]=gszTzxFile[i];
					   editstring (tempnaam);
					   if (tempnaam[0]==27) break;
                       prat (1,1,"Select TZX file to play:");
                       CloseTzxFile();
                       choose (9,tempnaam,&tapestate);
                       if (tempnaam[0]==27) {
                         gszTzxFile[0]=0;
                         break;
                       }
					   ...
					   vocplay=0;
                       loadpause=0;
                       p=open_file(tempnaam,_tapeinmode);
                       if ((int)p==-1) {
						   clearwindow();
						   prat (1,3,"File not found...");
						   prwind();
                       ...
                       for (i=0;i<129;i++) gszTzxFile[i]=tempnaam[i];
                       close_file(p);
					   srate=100;
					   vocdatapoint=0;
					   vocdatalen=0;
			  ...
			  switch (ch) {
                  ...
                  case 'r':
                       ...
                       // ALT-F7: keep old file for the moment
                       if ((tapeinh)&&(ch=='R')) {
                            close_file(tapeinh);
                       ...
                       if (tempnaam[0]==27) goto tapeagain;
					   addz80(tempnaam,"TAP");
                       i=open_file(tempnaam,_tapeinmode);
                       if (i==-1) {
						   clearwindow();
                           i=open_file(tempnaam,0);
                           if (i==-1) prat(1,2,"File not found..."); else {
                               close_file(i);
...
void save_prog(void)
{
      ...
      for (i=0;i<129;i++) oldnaam[i]=sprognaam[i];
      sprognaam[0]=0;
	  clearwindow();
      ...
      if (sprognaam[0]==0) {
          for (i=0;i<129;i++) sprognaam[i]=oldnaam[i];
          if (sprognaam[0]) {
              xcurs=1;ycurs=4;
              editstring(sprognaam);
          }
	  ...
	  if (handle!=0xFFFF) {
		  ...
		  prwind();
          ch=0xDF & getkeyfromlist (ynentlist);
...
int type;
{
      ...
      if ((loadpause||vocplay||gszTzxFile[0])&&(!type)) {    /* VOC/TZX playback mode? */
		  if (loadpause) {
			  lasttlo=tstates;
			  lastthi=tstatehi;
			  loadpause=0;
	  ...
	  if (x) {
          ...
          if ((x>=256+59)&&(x<=256+68)) tomenu='a'+x-256-59;
      }
...
int real_mode()
{
	  ...
	  if (winkey||vocplay) {
		  if (winkey) {
              ...
              prat (2,3,"playing VOC or TZX files.");
		  }
		  prwind();
		  do {} while (getkey());
		  do {} while (!getkey());
	  ...
	  do {
		  ...
		  switch(ch2) {
			  ...
			  case 27:
				   DISCD1Type=d1t;
				   DISCD1Drive=d1d;
                   if (d1t==-1) resetdrive(d1d);
				   DISCD2Type=d2t;
				   DISCD2Drive=d2d;
                   if ((d2t==-1)&&( (d1t!=-1)||(d1d!=d2d) )) resetdrive(d2d);
				   return;
				   /*
						  case 'Z':
...
void multi_level_load(void)
{
	  ...
	  if (handle==0xFFFF) {
          ...
          if (handle==0xFFFF) {
              ...
              prat (1,1,"Loading game level ");
              prgetal(level,1);
              prat(1,3,"Name of DAT or Z80/SLT file:");
              xcurs=1;
              ycurs=4;
              editstring(datname);
              ...
              do {
                  ...
                  hipos=0;
                  if (lopos==0xffffL) lopos=0x4000;  // v3.05 uncompressed blk
              ...
              if (regpointer->rhl>=16384) {
                  unpackfile(handle,&specpointer[regpointer->rhl],
                    levsiz,-regpointer->rhl);
              }
	  ...
	  do {
		  ...
		  switch (ch) {
			  ...
			  case 'S':
                   ...
                   if (((startadr<16384)||
                        (startadr+blocklength+(unsigned)-1<startadr))&&
                       (blocklength)) {
			  ...
			  case 'B':
				   ...
				   addr[0]='\005';
                   prat (1,ywmax-2,"'x' to clear. Prefix '#' for hex");
                   prat (1,3,"Enter address:  ");
				   editstring(addr);
                   debugadr=dechex(addr);
                   if (debugadr!=0) debugflg=0xFE; else debugflg=0;
				   break;
              case 'O':
                    logoutagain:
                    clearwindow();
