...
// copies Z80 file if fout!=0
{
    ...
    if (buffer.length > sizeof(buffer)-32) {
       printf ("ERROR!  Can only handle .Z80 files up to v3.05 (ie. secondary header\n54 bytes)\n");
       exit(1);
    }
    ...
    do {
        ...
        if ((i==3)&&(blockhdr.length!=0)) {
           if (blockhdr.length==-1) blockhdr.length=16384;
           if (fout)
...
char **str;
{
    printf ("ADDDAT - Add/Delete/Extract level/screen data to/from Z80/SLT file - v 4.00\n");
