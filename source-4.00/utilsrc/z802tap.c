...
int main(int argc, char **argv)
{
    ...
    puts ("Z802TAP - Converts .Z80 file to .TAP file - (c) 1999 G.A. Lunter - v4.00");
    ...
    if (header.pcreg!=0) {  /* prior to v2.0 */
      ...
      do {
         int toread;
         if ((int)block.length==-1) toread=16384; else toread=block.length;
         if (read(fin,temp,toread)!=toread) goto errz80;
         printf ("o");
         if ((int)block.length==-1)
            memcpy(specmem[block.page-3],temp,16384);
         else
            if (unpack(temp,specmem[block.page-3],16384)!=block.length) goto errz80;
      } while ((read(fin,&block,3)==3)&&(block.length));
    }
    if (header.hmode<4) {
