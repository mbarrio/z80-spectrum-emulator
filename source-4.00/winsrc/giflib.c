/* GIF compression routines, taken with permission from SCR2GIF.C
	by Martijn van der Heide of ThunderWare Research. */
...
// To store delay times, the variable gifLastExposureLoc holds the
//  file position of the 'delay' flag

extern fpos_t gifLastExposureLoc;
...
typedef struct
{
  ...
  byte        GlobalColourTableSize ;
  byte        SortFlag              ;
  byte        ColourResolution      ;
  byte        GlobalColourTableFlag ;
...
typedef struct
{
  ...
  word        Width;
  word        Height;
  byte        LocalColourTableSize ;
  byte        Reserved             ;
  byte        SortFlag             ;
  byte        InterlaceFlag        ;
  byte        LocalColourTableFlag ;
} _ImageDescriptor;
...
/*
static void	  GIFSetOutFile					  (FILE *file);
static byte   GIFCreate                    (char *FileName, short Width, short Height, short NumColours, short ColourRes, bool GIF89a);
static void   GIFSetColour                 (byte ColourNum, byte Red, byte Green, byte Blue);
static byte   GIFWriteGlobalColorTable     (bool Loop);
static byte   GIFCompressImage             (short StartX, short StartY, int Width, int Height,
															short (*GetPixelFunction)(short PixX, short PixY), bool GIF89a, word PictureDelayTime);
static byte   GIFClose                     (void);
...
/* Sets output file handle.  Should point to an empty file opened for writing. */
/* Call to _GIFCreate should have empty filename */
...

{
  ...
  if (_WriteByte (Tmp) != GIF_OK)
	 return (GIF_ERRWRITE);
  fgetpos(_OutFile,&gifLastExposureLoc);		// Added, GAL
...
byte GIFCreate (char *FileName, short Width, short Height, short NumColours, short ColourRes, bool GIF89a)
...
byte GIFCompressImage (short StartX, short StartY, int Width, int Height, short (*GetPixelFunction)(short PixX, short PixY),
