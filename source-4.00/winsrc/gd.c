...
extern fpos_t gifLastExposureLoc;		// dirty; see below and gif.c
...
static void GIFEncode (FILE *fp, int GWidth, int GHeight, int GInterlace, int Background, int Transparent, int BitsPerPixel, int *Red, int *Green, int *Blue, gdImagePtr im, char header, char loop, unsigned int time);
...
// Changed gdImageGif: added 'header' and 'time'.  See below
// Added loop: see below

void gdImageGif(gdImagePtr im, FILE *out, char header, char loop, unsigned int time)
{
	...
	GIFEncode(
		...
		im->red, im->green, im->blue, im, header, loop, time);
}
...
// GIFEncode changed: added 'header' and 'time'.  If !header, only the graphic
// gets written.  Also: no GIF file terminator (character ';') is written.

// Added loop.  If TRUE, a NETSCAPE extension block ("infinite loop") gets added.

// Very dirty trick: File position of time word gets written in gifLastExposureLoc

static void
GIFEncode(FILE *fp, int GWidth, int GHeight, int GInterlace, int Background, int Transparent, int BitsPerPixel, int *Red, int *Green, int *Blue, gdImagePtr im, char header, char loop, unsigned int time)
{
		  int B;
		  int RWidth, RHeight;
		  int LeftOfs, TopOfs;
		  ...
		  // Ah.  Voor filmpjes moet dit altijd GIF89a zijn!

//		  fwrite( Transparent < 0 ? "GIF87a" : "GIF89a", 1, 6, fp );

			if (header) {

				// Only write header if flag 'header' is true.
			  ...
			  // Now write out the extension block, if necessary

			  if (loop) {

				fwrite("\x21\xFF\x0BNETSCAPE2.0\x03\x01\x00\x00\x00", 1, 19, fp);
				// GIF extension code
				//  Appl extension label
				//  Appl block length
				//  Appl name
				//  Appl ID (2.0)
				//  Length sub-block (3)
				//   Loop count (word 0)
				//  Data sub-block terminator (0)
	...
	// Ah.  Dit blokje is ook nodig voor filmpjes

//	if ( Transparent >= 0 ) {
		 fputc( '!', fp );
		 fputc( 0xf9, fp );
		 fputc( 4, fp );
		 fputc( Transparent >= 0, fp );		// Transparency flag

		 // Dirty:
		 fgetpos( fp, &gifLastExposureLoc );
		 // End of dirty code

		 fputc( time & 0xff, fp );				// Delay time after display
		 fputc( time >> 8, fp );				// high order byte
		 fputc( (unsigned char) Transparent, fp );		// transparency index
		 fputc( 0, fp );		// terminator (zero-length block)
//	}
		  ...
		  // bit 7 is altijd 0 (geen local color table)
...
static long rl_count;
static long rl_table_pixel;
static long rl_table_max;
...
static unsigned long compute_triangle_count(unsigned long count, unsigned long nrepcodes)
{
 unsigned long perrep;
 unsigned long cost;
...
static void rl_flush_fromclear(long count)
{
 long n;
...
static void rl_flush_clearorrep(long count)
{
 long withclr;
...
static void rl_flush_withtable(long count)
{
 long repmax;
 long repleft;
 long leftover;
...
//							  continue;
							  return 0;			// Return: error! (Added GAL)
					}
...
GetCode_(FILE *fd, int code_size, int flag)
{
		 ...
		 if ( (curbit+code_size) >= lastbit) {
					...
					gcbuf[0] = gcbuf[last_byte-2];
					gcbuf[1] = gcbuf[last_byte-1];

					if ((count = GetDataBlock(fd, &gcbuf[2])) == 0)
							  done = TRUE;
